		...
		try {
			...
			client.player.displayClientMessage(
					Component.literal("已导出 " + result.itemCount() + " 个物品 ID、"
							...
							+ result.blockEntityCount() + " 个方块实体类型、"
							+ result.entityCount() + " 个实体 ID 和 "
							+ result.fluidCount() + " 个流体 ID 到 " + relativePath),
					false
			);
			Itemlister.LOGGER.info("Exported {} item IDs, {} block IDs, {} block entity types, {} entity IDs "
							+ "and {} fluid IDs to {}",
					result.itemCount(), result.blockCount(), result.blockEntityCount(), result.entityCount(),
					result.fluidCount(), result.outputFile());
